class Api::V1::ProductsController < ApplicationController
  ...
  before_action :set_product, only: %i[show update destroy history]
  ...
  def update
    if @product.update(product_params)
      generate_qr_code(@product) if @product.saved_change_to_name?
      render json: @product
    else
      render json: { errors: @product.errors.full_messages }, status: :unprocessable_entity
    end
  ...
  def history
    history_data = @product.stocks.order(:created_at).map do |stock|
      {
